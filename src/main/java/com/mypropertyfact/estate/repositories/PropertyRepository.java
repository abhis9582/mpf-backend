...
public interface PropertyRepository extends JpaRepository<Property, Integer> {
    ...
    @Query("SELECT p.id, p.projectName AS projectName, p.projectPrice AS price, p.projectLocality AS location, " +
            "MAX(pb.mobileBanner) AS image, p.slugURL AS slugURL " +
            "FROM Property p " +
            "JOIN ProjectBanner pb ON p.id = pb.projectId " +
            "WHERE pb.type = 'mobile' AND p.cityLocation = :cityName " +
            "GROUP BY p.id")
    List<Object[]> getAllByCity(@Param("cityName") String cityName);
}
